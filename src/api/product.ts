...
export const useGetProductCategoriesQuery = (filter?: unknown) => {
  ...
  const result = useQuery<
    { categories: ICategory[]; total: number; page: number; limit: number },
    Error
  >({
    queryKey: [QUERY_KEYS.Categories, paginationFilter],
    queryFn: async () => {
      const response = await https.get(
        "/product/categories" +
          `${stringifyQuery(paginationFilter as Record<string, string | string[] | number>)}`
      );
      return response.data.data;
    },
  });

  const item: IListItem[] = (result.data?.categories ?? [])?.map((e) => ({
    label: e.name,
    value: e._id,
  }));
