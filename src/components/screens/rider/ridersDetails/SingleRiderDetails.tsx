...
import { useGetSingleRiderByIdQuery } from "@/api/rider";
import RiderCard from "./RiderCard";

const SingleRiderDetails = () => {
  const { riderId } = useParams();
  const { data: singleRider } = useGetSingleRiderByIdQuery(riderId || "");

  // Use rider's location coordinates for the map
  const riderCoords: [number, number] = singleRider?.location?.coordinates
    ? [singleRider.location.coordinates[1], singleRider.location.coordinates[0]] // [latitude, longitude]
    : [6.6958313, 3.4222103]; // Fallback coordinates
  ...
  const handleShareAddress = async () => {
    const address = singleRider?.residentialAddress || "No address available";
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_500px] gap-6">
        ...
        <div className="grid gap-3">
          <div className="bg-white rounded-lg shadow-lg p-4">
            <div className="mb-8">
              <RiderCard singleRider={singleRider} />
            </div>
        ...
        <div className="grid gap-3">
            ...
            <div className="flex items-center gap-1">
              <div>
                <h3 className="flex items-center gap-3 text-lg text-[#656667]">
                  ...
                  <span>{singleRider?.phoneNumber || "N/A"}</span>
                </h3>
                <h3 className="pt-3 flex items-center gap-3 text-base text-[#656667]">
                  ...
                  <span>{singleRider?.email || "N/A"}</span>
                </h3>
              </div>
            </div>
          </ContactCard>
          <ContactCard title="Address">
            <div className="flex items-center gap-1">
              <div>
                <h3 className="flex items-center gap-3 text-md text-[#656667]">
                  <span>{singleRider?.residentialAddress || "N/A"}</span>
                </h3>
                <button
                  onClick={handleShareAddress}
                  className="mt-3 text-sm text-ctm-primary-700 underline"
                >
                  Copy Address
                </button>
              </div>
            ...
            <div>
              <div className="flex flex-col gap-4">
                <h3 className="font-normal flex flex-col gap-2 text-base capitalize">
                  ...
                  <span>{singleRider?.displayName || "N/A"}</span>
                </h3>
                <h3 className="font-normal flex flex-col gap-2 text-base capitalize">
                  <span className="font-semibold">Date of birth</span>
                  <span>
                    {singleRider?.dateOfBirth ? formatDOB(singleRider.dateOfBirth) : "N/A"}
                  </span>
                </h3>
                <h3 className="font-normal flex flex-col gap-2 text-base capitalize">
                  <span className="font-semibold">Account Status</span>
                  <span>{singleRider?.accountStatus || "N/A"}</span>
                </h3>
                <h3 className="font-normal flex flex-col gap-2 text-base capitalize">
                  <span className="font-semibold">Currently Working</span>
                  <span>{singleRider?.currentlyWorking ? "Yes" : "No"}</span>
                </h3>
              </div>
            </div>
          </ContactCard>
